...
let createEnterName = () => {
  ...
  var startpos = 325 + (name.length * lw * 0.5)
  console.log((name.length * 20), startpos)

  context.font = "32px alternian";
  ...
  for (let i = 0; i < name.length; i ++) {
    ...
    //context.fillStyle = "white"
    context.fillText(name[i], letterpos, 74)

    if (name[i + 1]) { 
      //context.fillRect(letterpos - (lw * 1.5), 78, 27, 5) 
  ...
  let drawneutral = text => {
    var newstartpos = 325 + (text.length * lw * 0.5)
    clearcanvas();
    drawbg();
    for (let i = 0; i < text.length; i ++) {
      var letterpos = newstartpos - ((i + 0.5) * lw)
      context.fillText(text[i], letterpos, 74)
    }
  }
  ...
  if (react == "yes") {
    ...
    drawneutral(name);
    context.drawImage(tick, startpos + 10, 38)
    ...
    drawneutral(name);
    context.drawImage(cross, startpos + 10, 38)
    gif.addFrame(canvas, {copy: true, delay: 50})
    drawneutral(name);
    gif.addFrame(canvas, {copy: true, delay: 50})
    context.drawImage(cross, startpos + 10, 38)
      ...
      for (let i = 0; i < name.length + 1; i ++) {
        ...
        for (let j = 0; j < 3; j ++) {
          for (let k = 0; k < 4; k ++) {
            ...
            context.fillRect(letterpos + (j * 10) - 6, 82 - ((k + 0.5) * 10), 10, 10)
          }
        }
      }
      gif.addFrame(canvas, {copy: true, delay: 50})
